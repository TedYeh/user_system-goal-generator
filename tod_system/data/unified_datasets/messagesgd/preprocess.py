...
def preprocess():
    original_data_dir = 'messagesgd'
    new_data_dir = 'data'

    if not os.path.exists(original_data_dir):
        original_data_zip = 'messagesgd.zip'
    ...
    for data_split in splits:
        data_dir = os.path.join(original_data_dir, data_split)
        # schema => ontology
        ...
        for root, dirs, files in os.walk(data_dir):
            fs = sorted([x for x in files if not 'schema' in x])
                ...
                for d in data:
                    ...
                    for utt_idx, t in enumerate(d['turns']):
                        ...
                        for frame in t['frames']:
                            ...
                            for action in frame['actions']:
                                ...
                                if action['act'] in ['REQ_MORE', 'AFFIRM', 'NEGATE', 'THANK_YOU', 'GOODBYE']:
                                    ...
                                    #print(prev_state,domain,intent,value_list[0])
                                    #if action['act'] == 'INFORM_INTENT':prev_state[domain]['intent'] = value_list[0]                                    
                                    ...
                                    if ontology['domains'][domain]['slots'][slot]['is_categorical']:
                                        ...
                                        for value in value_list:
                                            for slot_info in frame['slots']:
                                                ...
                                                #print(slot_info, slot, t['utterance'][start:end].lower(), value.lower())
                                                #input()
                        ...
                        if speaker == 'user':
                            ...
                            state[domain]['intent'] = frame['state']['active_intent']
                            active_intent = {}
                            requested_slots = {}
                            for frame in t['frames']:
                                domain = frame['service']
                                ...
                                #print(d['dialogue_id'], state, domain)

                ...
                if split_to_turn and speaker in [turn['speaker'], 'all']:
                    ...
                    if terminated:
                        sample['terminated'] = turn['utt_idx'] == len(
                            dialogue['turns']) - 1
                    #if len(sample['context'])>=context_window_size: data_by_split[data_split].append(sample)
...
def load_policy_data(dataset, data_split='all', speaker='system', context_window_size=0, use_context=False, **kwargs):
    """
    It loads the data from the specified dataset, and returns it in a format that is suitable for
    training a policy
